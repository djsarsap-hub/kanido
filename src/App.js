        ...
        <div style={s.wrap} className="tab-pane" key={tab}>
          ...
          {tab === "chest" && (
            <>
              ...
              <div style={s.card}>
                <span style={s.label}>Log a new treasure</span>
                <textarea style={s.textarea} placeholder="A task, spark, goal, or random thought..." value={newIdea} onChange={e=>setNewIdea(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&(e.metaKey||e.ctrlKey))addIdea();}}/>
                <button style={{ ...s.btn(), opacity:newIdea.trim()?1:.42 }} onClick={addIdea} disabled={!newIdea.trim()}>⚓ Log to Chest</button>
              </div>

              {/* Shuffle section — inline below add */}
              <div style={{ ...s.adventureCard, textAlign:"center", padding:"20px", marginBottom:14 }}>
                <div style={{ fontSize:11, color:C.muted, letterSpacing:2.5, textTransform:"uppercase", fontWeight:800, marginBottom:10 }}>🎲 Pick Your Next Quest</div>
                {shuffled ? (
                  <div style={{ fontFamily:"'Fredoka',sans-serif", fontSize:20, fontWeight:600, lineHeight:1.45, color:C.navText, marginBottom:14, animation:"popIn .35s ease" }}>
                    "{shuffled.text}"
                  </div>
                ) : (
                  <div style={{ color:C.muted, fontSize:13, fontWeight:600, lineHeight:1.6, marginBottom:14 }}>
                    {activeCnt === 0 ? "No loot yet — log something above! 📦" : `${activeCnt} treasure${activeCnt!==1?"s":""} in your hold`}
                  </div>
                )}
              ...
              {ideas.length === 0 ? (
                <div style={s.empty}>
                  <span style={s.emptyIcon}>🪙</span>
                  Your chest is empty.<br/>
                  <span style={{ opacity:.75 }}>Log your first treasure above, or try a suggestion!</span>
                </div>
              ) : (
                ideas.map(idea => (
                  <div key={idea.id} className="idea-row" style={{ ...s.item(idea.done), borderColor:idea.tag==="feel-good"&&!idea.done?C.accent2+"55":idea.done?"#B2DFDB":C.cardBorder }}>
          ...
          {tab === "bounty" && (
            <>
              ...
              <div style={{ ...s.adventureCard, textAlign:"center", padding:"22px 20px 18px" }}>
                ...
                <div style={{ fontSize:13, color:C.muted, lineHeight:1.7 }}>
                  Share your code so your crew can drop treasures and sparks into your chest. Keep what you like and leave the rest. A little wind in the sails goes a long way. 🌊
                </div>
              </div>
              ...
              <div style={s.card}>
                ...
                <p style={{ fontSize:13, color:C.muted, marginBottom:16, lineHeight:1.7 }}>
                  Give this code to your crew. They can drop tasks or motivation into your chest and you can do the same for them. Hunt for great ideas one at a time. 🦀
                </p>
                <div style={s.codeBox}>
                  <div style={{ fontSize:10, color:C.muted, letterSpacing:3, textTransform:"uppercase", marginBottom:8, fontWeight:700 }}>Your code</div>
                  <div style={s.codeVal}>{myCode}</div>
                  ...
                  <div key={box.code} style={s.card}>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
                      {editingFriend === box.code ? (
                        <div style={{ ...s.row, flex:1, marginRight:8 }}>
                          <input
                            style={{ ...s.input, letterSpacing:0, textTransform:"none", fontSize:13 }}
                            value={editingName}
